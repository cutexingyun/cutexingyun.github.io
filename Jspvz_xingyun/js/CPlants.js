var CPlants = NewO({
  ...
  oPeashooter = InheritO(CPlants, {
    ...
    AudioArr: [
      ...
      "shieldhit",
      "shieldhit2",
    ], // 设置 oPeashooter 对象的 AudioArr 属性为包含音频名称的数组
    PicArr: [
...
(oKernelPult = InheritO(oCabbage, {
  ...
  AttackAnim(ele, self, isButter) {
    ele.childNodes[1].src =
      self.PicArr[self.AttackGif + isButter] + "?" + this.id;
  },
  NormalAttack(zid) {
    var self = this;
    var ele = $(self.id);
    ...
    if (self.ButterChance > self.__proto__.ButterChance) {
      self.ButterChance = Math.max(
        self.ButterChance - Math.random() * 0.02,
        self.__proto__.ButterChance
      );
    } else if (self.ButterChance < self.__proto__.ButterChance) {
      self.ButterChance = Math.min(
        self.ButterChance + Math.random() * 0.02,
        self.__proto__.ButterChance
      );
    }
