...
oFootballZombie,oZomboni,oImp,oDuckyTubeZombie1,oSnorkelZombie,oBalloonZombie,oDolphinRiderZombie],
		...
		(function(d) {
			...
			switch (d) {
				case 0:
					...
					innerText(c, "哎呀!终于找到你了" + $User.Visitor.UserName + "。(点击继续)");
					break;
				case 1:
					...
					innerText(c, "那些僵尸不会减弱攻势啊！还有件事必须跟你说(点击继续)");
					break;
				case 2:
					PlayAudio("crazydavelong" + Math.floor(1 + Math.random() * 3));
					c.onclick = null;
					...
					innerText(c, "其实我早就知道我自己只是一个程序。(点击继续)");
					break;
				case 3:
					PlayAudio("crazydavelong" + Math.floor(1 + Math.random() * 3));
					c.onclick = null;
					...
					innerText(c,"但我还是会帮助你的！首先，你不能从天上得到阳光。天都黑了，肯定没阳光！(点击继续)");
					break;
				case 4:
					PlayAudio("crazydavelong" + Math.floor(1 + Math.random() * 3));
					c.onclick = null;
					...
					innerText(c, "你不要傻傻的期盼天上掉阳光。(点击继续)");
					break;
				case 5:
					PlayAudio("crazydavelong" + Math.floor(1 + Math.random() * 3));
					c.onclick = null;
					...
					innerText(c, "虽然这场战斗会很困难，但你要相信自己的运气，毕竟你都得到了小喷菇。(点击继续)");
					break;
				case 6:
					PlayAudio("crazydavelong" + Math.floor(1 + Math.random() * 3));
					c.onclick = null;
					...
					innerText(c, "对了，一定要记得带花盆和睡莲！(点击继续)");
					break;
				case 7:
	...
	AZ: [
      ...
      [oDancingZombie,2,4],
      [oImp,2,1],
      [oDuckyTubeZombie1,3,4],
      [oSnorkelZombie,3,5,[4]],
      [oBalloonZombie,2,5],
