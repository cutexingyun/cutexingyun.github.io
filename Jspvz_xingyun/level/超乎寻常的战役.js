...
ZName: [oBackupDancer, oImp, oZombie, oZombie2, oZombie3, oPoleVaultingZombie, oConeheadZombie, oBucketheadZombie, oNewspaperZombie1, oScreenDoorZombie, oFootballZombie, oDancingZombie, oZomboni, oJackinTheBoxZombie, oBalloonZombie, oSmallZombie, oSmallConeheadZombie, oSmallFootballZombie],
PicArr: ["images/Zombies/Zombie/1.gif", "https://s4.gifyu.com/images/background_TF.jpg", "https://s4.gifyu.com/images/background2_TF.jpg", "images/interface/background1unsodded2.jpg"],
backgroundImage: "https://s4.gifyu.com/images/background_TF.jpg",
CanSelectCard: 1,
DKind: 1,
LevelName: ["超乎寻常的战役", "道路绵延", "脑子保卫战", "多元塔防"][Math.floor(Math.random() * 4)],
...
UserDefinedFlagFunc: function() {
	if (oP.FlagZombies >= 25 && oP.FlagZombies <= 27) oP.SetTimeoutTomZombie([oZombie, oConeheadZombie]);
	if (oP.FlagZombies >= 37 && oP.FlagZombies <= 40) oP.SetTimeoutTomZombie([oPoleVaultingZombie, oFootballZombie, oDancingZombie]);
	if (oP.FlagZombies == oP.FlagNum) oP.SetTimeoutTomZombie([oDancingZombie, oJackinTheBoxZombie, oFootballZombie]);
	if (oP.FlagZombies == 21) {
		oS.ChangeBG($("Black_box"), $("BackGround_TF_night"), 1, function() {
	...
	if (oP.FlagZombies == 41) {
		oS.ChangeBG($("White_box"), $("BackGround_Unsodded"), 1, function() {
			...
			for (let i in oS.ZName) {
				let s = oS.ZName[i], p = s.prototype;
				p.ArR = [];
				for (let j = 1; j <= 5; j++) {
...
LoadAccess: function(start_game) {
	...
	delete oAudio['Glorious_Morning'];
	NewURLAudio({
		url: "https://music.163.com/song/media/outer/url?id=409941357.mp3",
		audioname: "Glorious_Morning",
		loop: true
	});
	init_dag();
	for (let i = 5; i >= 1; i--) {
...
ChangeBG: function(box, bg, opa, deffuc) {
	...
	oSym.addTask(1, function(index, kind, alpha) {
...
StartGame: function() {
	...
	PrepareGrowPlants(function() {
		PlayMusic(oS.LoadMusic = oS.StartGameMusic = "Glorious_Morning");
...
dag_traversal_of: function() {
	for (let _ in $Z) {
		let Eletop, b = $Z[_], r = b.R, point = (b.beAttackedPointR - b.beAttackedPointL) * 0.5, c = GetC(b.ZX + point), to = b.Next_Edge, now = '[' + r + ',' + c + ']', nowArr = [r, c];
		dag_zombie_init(b);
